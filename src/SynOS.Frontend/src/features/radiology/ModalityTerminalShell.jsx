    ...
    return (
        <div className="h-screen w-screen bg-zinc-950 text-zinc-100 flex flex-col font-sans select-none overflow-hidden">
            ...
            <div className="flex-1 grid grid-cols-12 overflow-hidden">
                ...
                <div className="col-span-4 border-r border-zinc-800 flex flex-col h-full bg-zinc-900/40">
                    <div className="p-4 border-b border-zinc-800 flex justify-between items-center bg-zinc-900/60">
                        <div className="flex items-center gap-2">
                            <Activity className="h-4 w-4 text-emerald-500 animate-pulse" />
                            <span className="font-bold text-sm uppercase tracking-wider text-zinc-300">Modality Worklist</span>
                        </div>
                        <button 
                            onClick={fetchQueue}
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-2">
                        {loading ? (
                            ...
                            <div className="h-full flex items-center justify-center flex-col p-8 text-center text-zinc-600">
                                <Database className="h-8 w-8 mb-2 text-zinc-700" />
                                <span className="text-xs font-semibold uppercase">No Pending Patients</span>
                                <span className="text-[10px] mt-1 text-zinc-600">All scans for this terminal are completed.</span>
                            </div>
                        ) : (
                            queue.map((study) => {
                                ...
                                return (
                                    <div 
                                        ...
                                        className={`p-3.5 rounded-lg border transition-all cursor-pointer ${
                                            isSelected 
                                                ? 'bg-emerald-950/20 border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.05)]' 
                                                : 'bg-zinc-900/60 border-zinc-800 hover:border-zinc-700 hover:bg-zinc-900'
                                        }`}
                                    >
                                        <div className="flex justify-between items-start mb-1.5">
                                            <span className="text-[10px] font-black uppercase bg-zinc-800 text-zinc-300 px-2 py-0.5 rounded tracking-wide">
                                        ...
                                        <h3 className="font-bold text-sm text-zinc-150 tracking-tight">{study.patientName}</h3>
                                        <div className="flex gap-3 text-[10px] text-zinc-400 mt-1">
                                            <span>Age: {study.patientAge}</span>
                                            <span>Gender: {study.patientGender}</span>
                                        </div>
                                        <div className="text-[11px] font-semibold text-zinc-300 mt-2 bg-zinc-950/40 p-1.5 rounded border border-zinc-800/40 flex items-center justify-between">
                                            <span>{study.testName}</span>
                                            <span className="text-zinc-500">{study.modality}</span>
                                        </div>
                                    </div>
                                );
                            })
                ...
                <div className="col-span-8 h-full flex flex-col overflow-hidden bg-zinc-950">
                    {activeStudy ? (
                            ...
                            <div className="p-4 border-b border-zinc-800 bg-zinc-900/20 flex justify-between items-center">
                                <div>
                                    <div className="flex items-center gap-2 mb-0.5">
                                        <span className="text-xs font-black uppercase text-emerald-400 bg-emerald-950/60 px-2 py-0.5 rounded border border-emerald-900">
                                            ACTIVE SESSION
                                        </span>
                                        <h2 className="font-bold text-base text-zinc-200">{activeStudy.patientName}</h2>
                                    </div>
                                    <p className="text-xs text-zinc-400">{activeStudy.testName} ({activeStudy.modality})</p>
                                </div>
                                {activeStudy.status === 'PendingImaging' && (
                                    <button
                                        onClick={() => handleAssign(activeStudy.radiologyStudyId)}
                                        disabled={actionLoading}
                                        className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white font-bold text-xs rounded uppercase tracking-wider flex items-center gap-1.5 transition-colors shadow-lg shadow-emerald-950/20"
                                    >
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                                {activeStudy.status === 'Assigned' ? (
                                    <div className="grid grid-cols-2 gap-6">
                                        {/* PACS Core Simulator */}
                                        <div className="bg-zinc-900/40 border border-zinc-800 rounded-xl p-5 flex flex-col justify-between min-h-[220px]">
                                            <div>
                                                <div className="flex items-center gap-2 mb-3">
                                                    <Cpu className="h-5 w-5 text-indigo-400" />
                                                    <h3 className="font-bold text-sm uppercase tracking-wider text-zinc-300">PACS Gateway Registry</h3>
                                                </div>
                                            ...
                                            {pacsStep === 0 ? (
                                                <button
                                                    ...
                                                    className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs uppercase tracking-wider rounded transition-colors flex items-center justify-center gap-1.5"
                                                >
                                                ...
                                                <div className="space-y-3 bg-zinc-950/60 p-4 rounded-lg border border-zinc-800">
                                                    <div className="flex justify-between text-[11px] font-bold uppercase tracking-wider">
                                                        ...
                                                        <span className={pacsStep === 4 ? "text-emerald-400" : "text-indigo-400"}>
                                                    ...
                                                    <div className="w-full bg-zinc-850 h-1.5 rounded-full overflow-hidden">
                                                        <div 
                                                            className={`h-full transition-all duration-500 ${pacsStep === 4 ? 'bg-emerald-500' : 'bg-indigo-500'}`}
                                                            style={{ width: `${(pacsStep / 4) * 100}%` }}
                                                        />
                                                    </div>

                                                    {pacsStep === 4 && (
                                                        <div className="text-[10px] text-zinc-400 space-y-1 pt-1.5 border-t border-zinc-850/60">
                                                            <div className="flex justify-between">
                                                                <span>Accession Number:</span>
                                                                <span className="font-mono text-zinc-300 font-bold">{pacsAccession}</span>
                                                            </div>
                                                            <div className="flex justify-between">
                                                                <span>Viewer Stream:</span>
                                                                <span className="font-mono text-indigo-400 hover:underline cursor-pointer truncate max-w-[200px]" onClick={() => window.open(pacsViewerUrl, '_blank')}>Orthanc Stream</span>
                                                            </div>
                                                        </div>
                                                    )}
                                                </div>
                                        ...
                                        <div className="bg-zinc-900/40 border border-zinc-800 rounded-xl p-5 flex flex-col justify-between min-h-[220px]">
                                            <div>
                                                <div className="flex items-center gap-2 mb-3">
                                                    ...
                                                    <h3 className="font-bold text-sm uppercase tracking-wider text-zinc-300">Manual DICOM Fallback</h3>
                                                </div>
                                                <p className="text-xs text-zinc-400 leading-relaxed mb-4">
                                            ...
                                            <div className="relative">
                                                ...
                                                <label 
                                                    ...
                                                    className={`w-full py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-bold text-xs uppercase tracking-wider rounded border border-zinc-700 transition-colors flex items-center justify-center gap-1.5 cursor-pointer ${actionLoading ? 'opacity-50 pointer-events-none' : ''}`}
                                                >
                                                ...
                                                {uploadProgress > 0 && (
                                                    <div className="mt-3 space-y-1.5 bg-zinc-950/60 p-3 rounded border border-zinc-800">
                                    ...
                                    <div className="bg-zinc-900/30 border border-dashed border-zinc-850 rounded-xl p-8 text-center text-zinc-500 flex flex-col items-center justify-center">
                                        <CloudLightning className="h-8 w-8 text-zinc-700 mb-2 animate-bounce" />
                                ...
                                {/* Completion Drawer */}
                                {activeStudy.status === 'Assigned' && (
                                    <div className="p-5 bg-emerald-950/15 border border-emerald-900/40 rounded-xl flex items-center justify-between">
                                        <div>
                                        ...
                                        <button
                                            ...
                                            className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs uppercase tracking-wider rounded transition-colors flex items-center gap-1.5 shadow-lg shadow-emerald-950/20 disabled:opacity-40"
                                        >
                        ...
                        <div className="h-full flex flex-col items-center justify-center p-8 text-center text-zinc-500">
                            <Monitor className="h-10 w-10 mb-2 text-zinc-700" />
                            <h3 className="font-bold text-sm uppercase tracking-wider text-zinc-400">Acquisition Workstation</h3>
