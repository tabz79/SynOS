...
export function ActionQueue({ columns, data, onAction }) {
    return (
        <div className="bg-zinc-900/50 border border-synos-border rounded-xl overflow-hidden flex-1 flex flex-col min-h-0">
            ...
            <div className="bg-white/5 border-b border-synos-border px-4 py-3 grid grid-cols-[1fr_2fr_1fr_1fr_minmax(100px,auto)] gap-4">
                {columns.map((col, idx) => (
            ...
            <div className="overflow-auto flex-1 p-2 space-y-1">
                {data.map((row, rowIdx) => (
                    <div key={rowIdx} className="bg-zinc-800/40 hover:bg-zinc-800/80 rounded-lg p-3 grid grid-cols-[1fr_2fr_1fr_1fr_minmax(100px,auto)] gap-4 items-center transition-colors group">
                        {/* Cell Rendering */}
